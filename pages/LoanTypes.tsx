...
const LoanTypes: React.FC = () => {
  ...
  // Set default filter to 'conventional' (standard home loan)
  ...
  const filteredLoans = LOAN_CATEGORIES.filter(loan => {
      ...
      if (filter === 'government') return loan.id === 'fha' || loan.id === 'va';
      if (filter === 'luxury') return loan.id === 'jumbo';
      if (filter === 'standard') return loan.id === 'conventional';
      return true;
  });

  const filters = [
    { id: 'all', label: 'All Mortgages' },
    { id: 'standard', label: 'Standard' },
    { id: 'government', label: 'Government-Backed' },
    { id: 'luxury', label: 'Luxury / Jumbo' },
  ];
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-950 min-h-screen">
      {/* Ticker Section with margin from navbar */}
      ...
      <div className="pt-4 pb-16 animate-fade-in">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-12">
            <h1 className="text-4xl lg:text-5xl font-serif font-bold text-slate-900 dark:text-white mb-6">Mortgage Products</h1>
            <p className="text-xl text-slate-600 dark:text-slate-400 max-w-2xl mx-auto mb-10">
              Find the perfect loan for your homeownership journey.
            </p>
          ...
          <div className="space-y-12 lg:space-y-24 min-h-[600px]">
            {filteredLoans.map((loan, index) => (
              ...
              >
                <div className="flex-1 space-y-8 w-full text-center lg:text-left">
                  <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-sm font-bold tracking-wide justify-center lg:justify-start">
                  ...
                  <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl text-left">
                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8 pb-8 border-b border-slate-100 dark:border-slate-800">
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 font-medium">
                            <DollarSign size={16} /> Max Loan Limit
                          </div>
                     ...
                     <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div className="text-center sm:text-left">
                          <span className="text-sm text-slate-500 dark:text-slate-400">Rates as low as</span>
          ...
          <div className="mt-32 mb-12 animate-fade-up">
             <div className="text-center mb-12">
                <h2 className="text-3xl font-serif font-bold text-slate-900 dark:text-white mb-4">Loan Comparison</h2>
                    ...
                    <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                      <thead className="bg-slate-50 dark:bg-slate-800/50">
                        <tr>
                          ...
                          {LOAN_CATEGORIES.map((loan) => (
                             <th key={loan.id} scope="col" className="p-6 text-left min-w-[180px]">
                                <div className="flex items-center gap-2 font-serif font-bold text-slate-900 dark:text-white text-base">
                                   {loan.title}
                                </div>
                             </th>
                          ))}
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800 bg-white dark:bg-slate-900">
                         <tr className="hover:bg-slate-50 dark:hover:bg-slate-800/20">
                            <td className="p-6 font-bold text-slate-700 dark:text-slate-300 bg-slate-50/30 dark:bg-slate-900/30">Min. Credit Score</td>
                            {LOAN_CATEGORIES.map((loan) => (
                               <td key={loan.id} className="p-6 text-slate-600 dark:text-slate-400">{loan.eligibility.replace('Credit Score ', '')}</td>
                            ))}
                         </tr>
                         <tr className="hover:bg-slate-50 dark:hover:bg-slate-800/20">
                            <td className="p-6 font-bold text-slate-700 dark:text-slate-300 bg-slate-50/30 dark:bg-slate-900/30">Down Payment</td>
                            {LOAN_CATEGORIES.map((loan) => (
                               <td key={loan.id} className="p-6 text-slate-600 dark:text-slate-400">{loan.features[0].includes('Down') ? loan.features[0] : 'Varies'}</td>
                            ))}
                         </tr>
                         <tr>
                            <td className="p-6 bg-slate-50/30 dark:bg-slate-900/30"></td>
                            {LOAN_CATEGORIES.map((loan) => (
                               <td key={loan.id} className="p-6">
